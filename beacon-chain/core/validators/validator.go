...
func InitiateValidatorExit(state *stateTrie.BeaconState, idx uint64) (*stateTrie.BeaconState, error) {
	...
	validator.WithdrawableEpoch = exitQueueEpoch.Add(params.BeaconConfig().MinValidatorWithdrawabilityDelay)
...
func SlashValidator(state *stateTrie.BeaconState, slashedIdx uint64) (*stateTrie.BeaconState, error) {
	...
	validator.Slashed = true
	validator.WithdrawableEpoch = types.MaxEpoch(validator.WithdrawableEpoch, currentEpoch+params.BeaconConfig().EpochsPerSlashingsVector)
...
func ExitedValidatorIndices(epoch types.Epoch, validators []*ethpb.Validator, activeValidatorCount uint64) ([]uint64, error) {
	...
	withdrawableEpoch := exitQueueEpoch.Add(params.BeaconConfig().MinValidatorWithdrawabilityDelay)
