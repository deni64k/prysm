...
func TestNextEpoch_OK(t *testing.T) {
	tests := []struct {
		...
		{slot: 0, epoch: types.ToEpoch(0/params.BeaconConfig().SlotsPerEpoch.Uint64() + 1)},
		{slot: 50, epoch: types.ToEpoch(0/params.BeaconConfig().SlotsPerEpoch.Uint64() + 2)},
		{slot: 64, epoch: types.ToEpoch(64/params.BeaconConfig().SlotsPerEpoch.Uint64() + 1)},
		{slot: 128, epoch: types.ToEpoch(128/params.BeaconConfig().SlotsPerEpoch.Uint64() + 1)},
		{slot: 200, epoch: types.ToEpoch(200/params.BeaconConfig().SlotsPerEpoch.Uint64() + 1)},
	}
	for _, tt := range tests {
		state, err := beaconstate.InitializeFromProto(&pb.BeaconState{Slot: tt.slot})
		require.NoError(t, err)
...
func TestVerifySlotTime(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				genesisTime: timeutils.Now().Add(-1 * 5 * time.Duration(params.BeaconConfig().SecondsPerSlot) * time.Second).Unix(),
				slot:        types.ToSlot(MaxSlotBuffer + 6),
			},
		...
		},
		{
			...
			args: args{
				...
				slot: types.ToSlot(((^uint64(0)) / params.BeaconConfig().SecondsPerSlot) + 24),
			},
			wantedErr: "is in the far distant future",
		},
	}
...
func TestValidateSlotClock_HandlesBadSlot(t *testing.T) {
	...
	assert.NoError(t, ValidateSlotClock(types.ToSlot(MaxSlotBuffer), uint64(genTime)), "unexpected error validating slot")
	assert.NoError(t, ValidateSlotClock(types.ToSlot(2*MaxSlotBuffer), uint64(genTime)), "unexpected error validating slot")
	assert.ErrorContains(t, "which exceeds max allowed value relative to the local clock",
		ValidateSlotClock(types.ToSlot(2*MaxSlotBuffer+1), uint64(genTime)), "no error from bad slot")
	assert.ErrorContains(t, "which exceeds max allowed value relative to the local clock",
		ValidateSlotClock(1<<63, uint64(genTime)), "no error from bad slot")
}
