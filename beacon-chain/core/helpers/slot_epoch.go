...
func SlotToEpoch(slot types.Slot) types.Epoch {
	return types.ToEpoch(slot.Div(params.BeaconConfig().SlotsPerEpoch.Uint64()).Uint64())
}
...
func ValidateSlotClock(slot types.Slot, genesisTimeSec uint64) error {
	maxPossibleSlot := types.ToSlot(CurrentSlot(genesisTimeSec).Uint64() + MaxSlotBuffer)
